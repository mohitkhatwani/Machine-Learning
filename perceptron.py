...
def perceptron(train_data,train_label,test_data,test_label,iteration):
    session = tf.Session()
    data_train = tf.placeholder(dtype=tf.float32,shape = [28*28])
    label_train = tf.placeholder(dtype= tf.float32,shape = [1])
...
def main():
    ...
    if choice == 'a':
        ...
        train_images,train_labels,test_images,test_labels = extract_data_shuffled(mnist.train.images,mnist.train.labels,mnist.test.images,mnist.test.labels,6000,1,6)
        accuracy = perceptron(train_images,train_labels,test_images,test_labels,10)[0]
        print("Accuracy with 12000 training examples",accuracy)
        accuracy_iteration(train_images,train_labels,test_images,test_labels,10,1,6)
        print("Plot for whole training examples plotted!!")
